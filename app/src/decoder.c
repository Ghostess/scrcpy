...
#ifdef HAVE_HWACCEL
    if (decoder->hwaccel) {
        LOGV("Decoder '%s': configuring hwaccel", decoder->name);
        if (!sc_hwaccel_configure_decoder(decoder->hwaccel,
                                         decoder->ctx,
                                         decoder->hwaccel_buffered_frames)) {
            LOGV("Decoder '%s': hwaccel configuration failed, falling back to software",
                 decoder->name);
        }
    }
#endif
